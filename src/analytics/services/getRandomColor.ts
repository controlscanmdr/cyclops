...
export function getRandomColor(): string {
  return randomcolor(BASE_COLOR_CONFIG) as string;
}
...
export function getRandomColorList(count: number, seed?: string): string[] {
  const extendedColorConfig = seed ? { seed, count } : { count };
  const colorConfig = { ...BASE_COLOR_CONFIG, ...extendedColorConfig };

  return randomcolor(colorConfig) as string[];
}
